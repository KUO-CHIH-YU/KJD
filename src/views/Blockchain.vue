...
async function query(id) {
  ...
  try {
    const tmpUrl = `https://cors-anywhere.herokuapp.com/${sApp.api}${inputDic.value[id]}` //https://cors-anywhere.herokuapp.com/
    if (id == 'btc') {
      ...
      const response = await axios.get(tmpUrl)
      if (id == 'tron') {
        if ('data' in response.data.data && response.data.data.length > 0) {
          rt = '✔️'
        } else {
          rt = '❌'
        }
